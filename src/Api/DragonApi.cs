...
public static class DragonApi {
    ...
    public static async Task<(string, string, string)> CreateDragonFromXML(HttpClient client, string childApiToken, string userID, XmlNode raisedPetData, int? xpValue, string? imageData, string? image2Data, bool removeDragonTickets) {
        ...
        string res1 = await SetRaisedPet(client, childApiToken, setRaisedPetRequest);
        string res2 = "no";
        try {
            if (xpValue != null) {
                res2 = await SetDragonXP(client, childApiToken, raisedPetData["eid"].InnerText, (int)xpValue);
            }
        ...
        string res3 = await ImageApi.SetImage(client, childApiToken, newPetXml.RaisedPetData.ImagePosition.Value, imageData);
        
        if (image2Data != null) {
            res3 += await ImageApi.SetImage(client, childApiToken, newPetXml.RaisedPetData.ImagePosition.Value, image2Data, "Mythie");
        }
